...
export const sendNotification = async (req, res) => {
  ...
  try {
    ...
    const [notificationResult] = await connection.query(insertNotification, [title, message]);
    ...
    if (Array.isArray(recipientIds) && recipientIds.length > 0) {
      ...
      await connection.query(insertRecipients, [values]);
      ...
      await connection.query(insertAll, [notificationId]);
      res.status(200).json({ message: "Notification broadcasted to all staff" });
    }
  } catch (err) {
    console.error("DB error (sendNotification):", err);
    return res.status(500).json({ error: "Database error" });
  }
};

// ✅ 2. Staff fetches their notifications (direct + broadcast)
export const getNotificationsForStaff = async (req, res) => {
...
export const markAllNotificationsRead = async (req, res) => {
  ...
  try {
    const connection = await getConnection();
    await connection.query(query, [userId, userId]);
...
export const deleteNotification = async (req, res) => {
  ...
  try {
    const connection = await getConnection();
    await connection.query(deleteReads, [id]);
    await connection.query(deleteRecipients, [id]);
    await connection.query(deleteNotification, [id]);
    res.status(200).json({ message: "Notification deleted" });
  } catch (err) {
    console.error("DB error (deleteNotification):", err);
...
export const updateNotification = async (req, res) => {
  ...
  try {
    const connection = await getConnection();
    await connection.query(query, [title, message, id]);
